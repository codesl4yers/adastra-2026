...
def _fragmentar_con_estadisticas(
    ...
    for seccion in _agrupar_secciones(bloques, config):
        ...
        for oraciones, tiene_solape, tipo_unidad in grupos:
            fragmentos.append(
...
) -> list[tuple[list[_Oracion], bool, str]]:
    """Capas 2 a 4 sobre una sección de prosa."""
    oraciones = _oraciones_de(seccion.bloques, documento.idioma)
    if not oraciones:
        return []
...
def _grupos_atomicos(
    ...
    """
    grupos: list[tuple[list[_Oracion], bool, str]] = []
    ...
    for registros in _agrupar_registros(seccion.bloques, config):
        ...
        oraciones = _oraciones_de_texto(texto, documento.idioma, registros[0].pagina)
        if not oraciones:
            continue

        if contar_palabras(texto) <= config.max_palabras:
            grupos.append((oraciones, False, "atomico"))
            continue

        estadisticas["atomicos_partidos"] += 1
        for trozo in _empaquetar(oraciones, config, presupuesto):
            grupos.append((trozo, False, "atomico"))
...
def _oraciones_de(bloques: Iterable[Bloque], idioma: str) -> list[_Oracion]:
    ...
    for bloque in bloques:
        for texto in segmentar(bloque.texto, idioma):
            oraciones.append(
...
def _oraciones_de_texto(texto: str, idioma: str, pagina: int | None) -> list[_Oracion]:
    return [
        _Oracion(texto=trozo, pagina=pagina, es_titulo=False)
        for trozo in segmentar(texto, idioma)
    ]

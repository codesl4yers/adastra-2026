...
def extraer(path: Path, fenomeno: int) -> Documento:
    ...
    try:
        with pdfplumber.open(str(path)) as documento_pdf:
            ...
            if _parece_escaneado(lineas_por_pagina):
                return _extraer_por_ocr(documento_pdf, path, fenomeno, meta)

            bloques = _bloques_de_lineas(lineas_por_pagina)
    ...
    errores = [] if any(bloques) else ["el PDF no produjo ningún bloque de texto"]
...
def _bloques_de_lineas(lineas_por_pagina: list[list[Linea]]) -> list[Bloque | None]:
    descartables = _descartables(lineas_por_pagina)
    tamanos = [linea.tamano for lineas in lineas_por_pagina for linea in lineas]
...
def _extraer_por_ocr(documento_pdf, path: Path, fenomeno: int, meta: dict[str, Any]) -> Documento:
    """Rasteriza y reconoce, o explica por qué no puede.

    Solo se llega aquí con un PDF sin capa de texto. Sin Tesseract el documento
    sale vacío pero marcado, para que el reporte pueda listarlo y una corrida
    posterior lo recupere sin volver a procesar el corpus entero.
    """
    ...
    if not ocr.hay_ocr():
        return construir_documento(
            ...
            bloques=[],
            meta=meta,
            errores=[f"PDF sin capa de texto. {ocr.motivo_sin_ocr()}"],
        )
    ...
    paginas = documento_pdf.pages[:MAXIMO_PAGINAS_OCR]
    if len(documento_pdf.pages) > MAXIMO_PAGINAS_OCR:
    ...
    for numero, pagina in enumerate(paginas, start=1):
        ...
        for linea in texto.splitlines():
            if not es_ruido_estructural(linea):
                bloques.append(jerarquia.ocr(linea, pagina=numero))

    if confianzas:
        meta["confianza_ocr"] = round(sum(confianzas) / len(confianzas), 2)

    errores = [] if any(bloques) else ["PDF sin capa de texto y el OCR no reconoció nada"]
    return construir_documento(
        fuente=path.name,
        formato=FORMATO,
        fenomeno=fenomeno,
